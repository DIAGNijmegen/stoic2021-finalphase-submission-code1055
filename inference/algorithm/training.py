...
from data.zip_loader import ZipLoader

import evaluate
from data.siamese import SiameseLoader
from misc_utilities.tensorboard import SummaryWriter, log_cv_tensorboard_summary, log_cv_tensorboard_bestsummary
from misc_utilities import determinism
from misc_utilities.plot import plot_conf_mat
from config.config import BaseConfig
from config.modelconfig import get_model, get_optimizer
from config.dataconfig import get_dataset
from time_iterator import itertime
from misc_utilities.git import write_git_log
from loss import BinaryCrossEntropySevOnly, assert_siamese_compatibility, get_loss
from misc_utilities import deactivate_batchnorm
...
class Trainer:
    def __init__(self, config: BaseConfig, log_directory=None, datasets=None, dataconfigs=None, no_tqdm=False):
        ...
        with open(self.output_dir/"dataset.json", "w") as data_json:
            json.dump(dict(
                train=self.datasets["train"].tfm_config(),
                val=self.datasets["val"].tfm_config(),
        ...
        if modelcfg.lr_decay is not None and (modelcfg.cosine_decay is None):
            ...
            T_max = self.datasets["train"].__len__() // self.config.Batch_SIZE * self.config.MAX_EPOCHS
    ...
    def epoch(self, epoch_num: int):
        ...
        metrics = evaluate.evaluate(self.val_loader, self.model, val_loss_fn, self.model_uses_metadata, device=self.config.DEVICE, no_tqdm=self.no_tqdm)
        self.dump_predictions(metrics["all_data"], "pred", epoch_num)
        if self.config.use_ema:
            ema_metrics = evaluate.evaluate(self.val_loader, self.model_ema, val_loss_fn, self.model_uses_metadata, device=self.config.DEVICE, no_tqdm=self.no_tqdm)
    ...
    def save_checkpoint(self, checkpoint_dir, epoch_num=None, metrics=None):
        ...
        if self.config.keep_additional_checkpoints > 0:
            ...
            for cpath in Path(checkpoint_dir).glob("checkpoint-*.pt"):
                ...
                if best_metric is None or ckp["metrics"]["auc_sev2_ema"] > best_metric:
                    best_metric = ckp["metrics"]["auc_sev2_ema"]
                    best_epoch = epoch
            del ckp
        ...
        # aggregate best checkpoints for all folds
        logging.info("Aggregating checkpoints")
        fold_checkpoints = {}
        for rundir in self.output_dir.parent.parent.iterdir():
            if not rundir.is_dir():
                continue
            for folddir in rundir.iterdir():
                foldid = folddir.name
                if not foldid.startswith("cv"):
                    continue

                best_checkpoint = fold_checkpoints.get(foldid, None)
                best_fold = None

                for cpath in folddir.glob("*.pt"):
                    checkpoint = torch.load(cpath, map_location="cpu")
                    if best_checkpoint is None or (checkpoint["metrics"]["auc_sev2_ema"] > best_checkpoint["metrics"]["auc_sev2_ema"]):
                        best_checkpoint = checkpoint
                        best_fold = cpath.stem

                if best_checkpoint is not None:
                    fold_checkpoints[foldid] = best_checkpoint
                    logging.info(f"Best checkpoint for fold {foldid}: {rundir.name}/{best_fold}")

        # create checkpoint backup just in case torch.save is interrupted when the container is killed
        # leading to a corrupted checkpoint.pt file
        tstamp = datetime.now().isoformat()
        prefix = self.output_dir.parent.name.split("_")[0]
        ckp_path = Path(f"/output/{prefix}-{tstamp}.pt")
        old_ckps = list(Path("/output").glob(f"{prefix}-*.pt"))
        torch.save({
            "config": self.config.to_dict(),
            "data_transforms": self.datasets["val"].tfm_config(),
            "models": [ckp["model_ema"] for ckp in fold_checkpoints.values()],
        }, ckp_path)
        # delete old aggregated checkpoints from this run
        # AFTER the new checkpoint was successfully written
        for old_path in old_ckps:
            old_path.unlink()
    ...
    def write_tensorboard_metrics(self, metrics, epoch_num):
        self.writer.add_scalar("ROC_AUC/val_inf", metrics["auc_inf"], epoch_num + 1)
        self.writer.add_scalar("ROC_AUC/val_sev", metrics["auc_sev"], epoch_num + 1)
        self.writer.add_scalar("ROC_AUC/val_sev2", metrics["auc_sev2"], epoch_num + 1)
        self.writer.add_scalar("Loss/val", metrics["val_loss"], epoch_num + 1)
        self.writer.add_scalar("Loss/val_sev=0", metrics["val_loss_sev0"], epoch_num + 1)
        self.writer.add_scalar("Loss/val_sev=1", metrics["val_loss_sev1"], epoch_num + 1)

        if self.config.use_ema:
            self.writer.add_scalar("ROC_AUC/val_inf_ema", metrics["auc_inf_ema"], epoch_num + 1)
            self.writer.add_scalar("ROC_AUC/val_sev_ema", metrics["auc_sev_ema"], epoch_num + 1)
            self.writer.add_scalar("ROC_AUC/val_sev2_ema", metrics["auc_sev2_ema"], epoch_num + 1)
            self.writer.add_scalar("Loss/val_ema", metrics["val_loss_ema"], epoch_num + 1)
            self.writer.add_scalar("Loss/val_ema_sev=0", metrics["val_loss_ema_sev0"], epoch_num + 1)
            self.writer.add_scalar("Loss/val_ema_sev=1", metrics["val_loss_ema_sev1"], epoch_num + 1)
...
def main():
    ...
    config = BaseConfig(args.model, num_steps=args.num_steps, cv_enabled=args.cv, num_trained_stages=args.num_trained_stages, nickname=args.nick,
                        split="../splits/split_sev_cv5.csv", data_name="gpudeformed")
