...
from segmentation.data.datasets import SupervisedDataset, UnsupervisedDataset_stoic, ValidationDataset, UnsupervisedDataset_tcia
...
def run(config, args):
    ...
    #identifier = 'test'
    supaug = '_supaugTrue' if config.SUPERVISED_AUGMENTATIONS else ''
    ...
    normalize_tcia = Normalize3D(mean=0.2248670, std=0.3062496) if config.DO_NORMALIZATION else My_Identity()
    normalize_stoic = Normalize3D(mean=0.3058265, std=0.3201640) if config.DO_NORMALIZATION else My_Identity()
    ...
    unsup_dataset_stoic = UnsupervisedDataset_stoic(path=config.DATA_PATH, transform=transform_unsupervised_stoic)
    unsup_dataset_tcia = UnsupervisedDataset_tcia(path=config.DATA_PATH, transform=transform_unsupervised_tcia)
    ...
    if config.SUPERVISED == False:
        unsup_dataset = unsup_dataset_stoic if config.UNSUPERVISED_DATA == 'stoic' else unsup_dataset_tcia
