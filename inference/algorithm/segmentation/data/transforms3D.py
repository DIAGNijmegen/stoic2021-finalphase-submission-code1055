...
def get_transform(imsize, normalize=None):
    zoomsize = 224 if imsize == '256' else 112
    insize = 256 if imsize == '256' else 128
    transform = Compose([
        Rand(Compose([Zoom((insize, insize, insize)), Crop((zoomsize, zoomsize, zoomsize), (insize, insize, insize))]), Zoom((zoomsize, zoomsize, zoomsize)), prob=0.5),
